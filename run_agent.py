...
class AIAgent:
    ...
    def _run_turn_with_cost_router(self, user_message, run_turn, persist_user_message):
        """Run one turn through the opt-in cost-cascade router.

        ``run_turn(persist)`` executes the conversation loop for this turn and
        returns its result dict; it may be called more than once when
        escalation is enabled. The agent's configured model is overridden for
        the duration and always restored.

        When the router is disabled (the default) this is a single
        pass-through call, so the normal model-selection path is untouched.
        """
        router = self._get_cost_router()
        if router is None:
            return run_turn(persist_user_message)

        # Tier classification is keyword-based over the user's text. A
        # multimodal turn arrives as a list of content parts, and flattening it
        # to "" would classify a vision request as SIMPLE and route it to the
        # cheapest rung. Bypass the router instead of guessing.
        ...
        try:
            ...
            with bind_subagent_parent(self), scoped_runtime_main({}):
                try:
                    ...
                    # Opt-in cost-cascade router. Disabled by default, in which
                    # case this is a single pass-through call to _run_turn.
                    # Deliberately INSIDE the durable turn lease / relay turn:
                    # an escalation retry is a second pass of the same logical
                    # turn, so it must not re-acquire the lease or open a second
                    # relay turn.
                    #
                    # getattr, not a plain attribute: this method is also driven
                    # with duck-typed stand-ins for ``self`` (tests, external
                    # relay shims). Those never opt into routing, and must not
                    # trip an AttributeError on the way to the loop.
                    _route_turn = getattr(self, "_run_turn_with_cost_router", None)
                    if _route_turn is None:
                        result = _run_turn(persist_user_message)
                    else:
                        result = _route_turn(
                            user_message, _run_turn, persist_user_message
                        )
                finally:
