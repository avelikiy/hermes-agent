...
def main():
    ...
    # Decoding happens inside the read below, outside the try further down, so
    # a single undecodable byte on the command pipe raised UnicodeDecodeError
    # and killed the whole gateway process — the try only ever guarded
    # json.loads. Replacing bad bytes turns that into an ordinary parse error,
    # which this loop already knows how to answer.
    try:
