...
class TestRunJobScript:
    ...
    def test_non_windows_script_preserves_default_text_decoding(self, cron_env, monkeypatch):
        ...
        # Fork divergence from upstream, which asserts no `errors` kwarg here.
        # A cron script is arbitrary user code and its stdout is not
        # guaranteed UTF-8 — mail-triage scripts read messages in
        # windows-1251 / latin-1 and echo those bytes. With strict decoding
        # `communicate()` raises UnicodeDecodeError, the outer handler turns
        # every such run into "Script execution failed: 'utf-8' codec can't
        # decode byte 0xa3", and the job silently never produces output.
        # `errors="replace"` is still POSIX-only-agnostic: the Windows branch
        # (creationflags + encoding) remains Windows-only, which is what this
        # test exists to pin.
