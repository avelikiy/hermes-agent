...
def run_conversation(
    ...
    if final_response is None and _turn_exit_reason == "cost_limit_reached":
        # Explain the stop instead of summarising it. finalize_turn's
        # iteration-exhaustion path spends one more API call to have the model
        # summarise; here that would be self-defeating — the cap exists to stop
        # spending, and a turn cut for cost must not cost extra to report.
        # Setting final_response now also keeps that path from firing at all
        # (it is gated on ``final_response is None``). Without this the caller
        # gets an empty response and the run looks hung rather than
        # deliberately halted.
