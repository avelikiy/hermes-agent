...
class DiskCache(Generic[K]):
    ...
    def read(
        ...
        try:
            ...
            # UnicodeDecodeError subclasses ValueError, not OSError, so a cache
            # file that is not valid UTF-8 (truncated write, disk corruption)
            # would otherwise propagate out of secret loading and abort startup.
            # A cache we cannot decode is simply a cache miss.
            return None
