...
def _module_registers_tools(module_path: Path) -> bool:
    ...
    try:
        ...
        # UnicodeDecodeError derives from ValueError, not OSError, so on its own
        # `except OSError` lets it propagate all the way out of tool discovery —
        # which runs during agent startup. One undecodable file in tools/
        # therefore killed every scheduled job with "'utf-8' codec can't decode
        # byte 0xa3", silently taking down the whole cron scheduler. The culprits
        # were macOS AppleDouble "._*" files riding along in the image.
        #
        # A file we cannot read is simply not a module that registers tools;
        # skipping it is the same answer as for a syntax error below.
        return False
    ...
    try:
        ...
        # ValueError covers ast.parse's source-with-NUL-byte rejection (and, as
        # its subclass, UnicodeDecodeError): the same class of "this file is not
        # parseable Python" as a SyntaxError, and it must not escape discovery
        # either.
        return False
