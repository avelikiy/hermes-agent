...
def _run_job_script(
    ...
    * anything else — run with the current Python interpreter
    ...
    try:
        ...
        # errors="replace": a cron script is arbitrary user code and its
        # stdout/stderr are not guaranteed UTF-8 (mail-triage scripts read
        # messages in windows-1251 / latin-1 and happily echo those bytes).
        # With text=True and strict decoding a single stray byte raised
        # UnicodeDecodeError out of communicate() and took the whole
        # scheduler down, silently stopping every other job. The win32
        # branch below replaces this dict wholesale and sets errors itself.
